...
class Highlight {
  ...
  List<Mode> _expandMode(Mode mode) {
    ...
    return mode.cached_variants ?? (mode.endsWithParent == true ? [Mode.inherit(mode)] : [mode]);
  }

  RegExp _langRe(String value, [bool? global]) {
    return RegExp(value, multiLine: true, caseSensitive: _languageMode!.case_insensitive != true);
  }
  ...
  void _compileMode(Mode mode, [Mode? parent]) {
    ...
    if (mode.keywords != null) {
      ...
      void _flatten(String className, String? str) {
        ...
        str!.split(' ').forEach((kw) {
          var pair = kw.split('|');
          try {
            compiledKeywords[pair[0]] = [className, pair.length > 1 ? int.parse(pair[1]) : 1];
          } catch (err) {
            print(err);
            // FIXME:
          }
    ...
    if (parent != null) {
      ...
      if (mode.endsWithParent == true && parent.terminator_end != null) {
        mode.terminator_end = mode.terminator_end! + (mode.end != null ? '|' : '') + parent.terminator_end!;
      }
    }
    ...
    }).toList()..addAll([mode.terminator_end, mode.illegal])).where((x) => x != null && x.isNotEmpty).toList();

    mode.terminators = terminators.isNotEmpty ? _langRe(_joinRe(terminators, '|'), true) : null;
  }

  List<Node>? _buildSpan(String? className, List<Node>? insideSpan, {bool noPrefix = false}) {
    if (!_classNameExists(className)) {
      return insideSpan;
    }

    return [Node(noPrefix: noPrefix, className: className, children: insideSpan)];
  }
  ...
  RegExp _escapeRe(String value) {
    return RegExp(value.replaceAll(RegExp(r'[-\/\\^$*+?.()|[\]{}]'), r'\$&'), multiLine: true);
  }
    ...
    for (var i = 0; i < mode.contains!.length; i++) {
      if (_testRe(mode.contains![i]!.beginRe, lexeme)) {
        if (mode.contains![i]!.endSameAsBegin == true) {
          mode.contains![i]!.endRe = _escapeRe(mode.contains![i]!.beginRe!.firstMatch(lexeme)![0]!);
        }
  ...
  void _addNodes(List<Node> nodes, List<Node>? result) {
    nodes.forEach((node) {
      if (result!.isEmpty || result.last.children != null || node.className != null) {
        result.add(node);
      } else {
  ...
  Result parse(String source, {String? language, bool autoDetection = false, String? subLanguage}) {
    if (language == null) {
      if (autoDetection) {
    ...
    return _parse(source, language: language, subLanguage: subLanguage);
  }

  Result _parse(String source, {String? language, bool ignoreIllegals = false, Mode? continuation, String? subLanguage}) {
    var langMode = _languageMode = _getLanguage(language) ?? plaintext;

    if (subLanguage != null && allLanguages.containsKey(subLanguage)) {
      jinja.subLanguage = [subLanguage];
      registerLanguage(subLanguage, allLanguages[subLanguage]!);
    }
    // TODO: strategy
    ...
    dynamic _keywordMatch(Mode mode, RegExpMatch match) {
      var match_str = langMode.case_insensitive == true ? match[0]!.toLowerCase() : match[0];
    ...
    List<Node> _processKeywords() {
      ...
      while (match != null) {
        ...
        if (keyword_match != null) {
          ...
          _addNodes(_buildSpan(keyword_match[0], [Node(value: match[0])])!, result);
        } else {
          _addText(match[0], result);
        }
        last_index = match.start + match[0]!.length;
        match = top!.lexemesRe!.allMatches(mode_buffer, last_index).firstWhereOrNull((m) => true);
      }
    ...
    List<Node>? _processSubLanguage() {
      ...
      var result = explicit
          ? _parse(mode_buffer, language: top!.subLanguage!.first, ignoreIllegals: true, continuation: continuations[top!.subLanguage!.first])
          : _parseAuto(mode_buffer, languageSubset: top!.subLanguage!.isNotEmpty ? top!.subLanguage : null);
    ...
    void _processBuffer() {
      _addNodes(top!.subLanguage != null ? _processSubLanguage()! : _processKeywords(), currentChildren);
      mode_buffer = '';
    }
    ...
    try {
      ...
      // print(value);
      while (true) {
        match = top!.terminators?.allMatches(source, index).firstWhereOrNull((m) => true);
      ...
      // print(relevance);
      // print(result);
      return Result(language: language, relevance: relevance, nodes: currentChildren, top: top);
    } catch (e) {
  ...
  Result _parseAuto(String source, {List<String>? languageSubset}) {
    languageSubset = languageSubset ?? _languages.keys.toList(); // TODO: options
