...
class JinjaService {
  ...
  final DebugController _debugController = DebugController()..enabled = true;
  ...
  Future<String> renderJinjaTemplate({
    ...
    for (final breakpoint in breakpoints) {
      _debugController.addLineBreakpoint(breakpoint);
    }

    final Map<String, dynamic> newBreakpointsInfo = {};

    _debugController.onBreakpoint = (info) async {
      final count = newBreakpointsInfo.keys
          .where((element) => element.contains(info.lineNumber.toString()))
          .length;
      ...
      newBreakpointsInfo['${info.lineNumber}_$count'] = {
        ...
        'lineNumber':
            count == 0 ? info.lineNumber : '${info.lineNumber}\nloop count: ${count + 1}',
    ...
    return await _templateOfJinja.renderDebug(
